...
package Assets {
	...
	public class BigPlatformEnemy extends Objects{
		...
		/**Constructor*/
		public function BigPlatformEnemy(xPos:Number, yPos:Number, width:Number, height:Number, type:int,  startingDirection:int, _settings:SharedObject){
				...
				if(platformEnemyClip.dead){
					...
					if(Math.random() > 0.9){
						...
						if(Math.random() > 0.7 && Player.playerHealth < 6){
							var healthDrop:ItemDrop = new ItemDrop(collisionBody.GetPosition().x, collisionBody.GetPosition().y, 1.5, 1.5, 1, settings);
						}
						else{
